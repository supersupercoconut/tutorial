...
- [x] leetcode 20道动态规划即可
    - 343 整数拆分（这个比较困难，耽误了不少时间）
    - 96 不同的二叉搜索树 思路比较巧妙
- [ ] 确定《大话数据结构》以及《大话设计模式》
- [ ] vinsfusion以及groundfusion逻辑或者mins逻辑 (需要确定一下mins中的插值与cocolic中的插值是不是同一种的东西)
    - 周一 配置完成mins，并且可以在自己的数据集上运行，但是算法的精度需要进一步提升
- [x] 上周没有实现出来的一些小问题
- [x] 录制室外数据（两周之内要完成所有数据集的录制工作，这样可以保留足够的时间冲刺算法改进）
- [x] 想起来还有SB的PPT没有弄..
...
- [x] 打电话转换学分(需要确定一下)
- [x] leetcode刷题 (仍然是一天三道)
    - 518 零钱兑换
    - 卡码网 52
    - 377 组合总和
- [x] 转换脚本
    - [x] evo转换轨迹数据转换到真实数据(需要原点对齐，即对应了--align_origin命令) | 确认轮计数据并且转换成为轨迹数据 | 轨迹对比确认的录制的场景是一种退化场景
- [x] 解决GPS无数据问题
...
- [x] 培养方案 -—— 有一个哲学方法论一直给我显示必修，明天得在系统群里面询问一下
- [x] leetcode 三道
    - 爬楼梯 卡码网 57
    - 322 零钱兑换
    - 279 完全平方数
- [x] 录制lidar室内数据集(这个得等到比较晚的时候才能录制)，但是可以用标定码跟fastlio先测试一下，就找最后一帧的数据来处理即可
- [x] wheel与动补坐标系对齐 - 周一的脚本里面并没有直接对齐(目前只考虑xy坐标转换，还是没有办法对齐) 
    - 分析结束之后发现之前录制的数据里面wheel都很漂移...轮计问题感觉非常大hhh
...
- [x] leetcode 三道题
    ...
    - 300
- [ ] 结束视觉部分录制
- [ ] 整理一组测试工具 —— 即测试这些数据集是否能给目前选择好的算法使用 (如果正常的时候不如直接使用docker创建环境来解决，要不然一直碰到anaconda与本地环境之间的冲突，很麻烦....)
...
1. 全部上电启动电脑 —— 这样串口工具是work的
2.  电脑程序先启动ublox才能再启动px4.launch\
3. 每次重新录制数据的时候 —— 冲洗启动ublox以及odom
4. GPS数据需要保证出现五个才是正常的
...
- [x] vins光流检测特征点(之前是直接使用vins中的源程序进行处理的, 但是最后感觉还是最终单独农民工出来比较好)
- [x] 卡码网至少三道(感觉图论问题是比较easy) 确实easy
- [x] 开题随便写个3~4页(毕竟本周要弄完)
- [x] 下午录制动态数据集 - ublox也需要录制下，现在需要测试天线是否符合要求(需要与陶瓷天线对比) —— 测试之后的结果就是陶瓷天线的效果比较好，应该说相当好！！！所有的话题全部都是齐全的
- [x] 大胖实验uwb的安装
- [x] 车要充电！！！！一定要充电
...
- [ ] 随想录 三道题
- [ ] 开题报告
- [ ] 恢复之前的rosbag环境, 输出RTK轨迹分析一下
- [ ] 有时间测试下ublox输出的数据能否使用
