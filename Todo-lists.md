...
- [ ] 学一下gstam的使用(发现没有使用位姿估计的协方差也可以解决问题)

- [x] 尝试用原版的voxel来进行lvisam的适配
    - 原版对应voxel效果不错的，使用起来比较简单。但是现在的问题是voxelmap++对应的方法，使用的是在M2DGR上对应出来的数据集，单独运行时候效果还不错的，为什么做了里程计适配之后效果不好了
    - 分点时间去做整理思路，剩余的时间去看mesh生成
    
- [x] 解决了所有的voxelmap的实现，已经集于voxelmap++做完了lvisam的适配工作，开始做mesh图的重建工作

    - 如果真的想形成一个voxelmap的类还是可以实现的(估计要慢慢的将所有的类成员函数什么做一些修改)

        

做里程计适配的voxelmap++效果还是不错的(没有考虑里程计中对于R与t的误差)
