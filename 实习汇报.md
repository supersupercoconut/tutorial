...
- **以FAST-LIO1为例，主要是分析IMU递推方程/LiDAR测量方程以及/去除运动畸变三者**。
  ...
  - IMU测量模型：IMU测量得到的角速度/加速度都是针对当前时间的Body系(或者是叫IMU系)的测量结果，不过由于加速度计会受到重力加速度的影响，在FAST-LIO中会通过静止100帧获取到一个重力加速度的估计值(即获取到全局系G中的重力值$g^G$), 计算$a_m$就非常方便。
    $$
    \begin{aligned}
  ...
  - IMU递推方程: IMU作为LiDAR帧之间的状态估计工具，会涉及到状态以及协方差的递推。状态方面会选择误差状态向量以及名义状态量同时参与推导。**名义状态就是在递推过程中不包含噪声的估计结果, 误差状态向量即作为真实状态与名义状态之间的差值**。名义状态会在每次计算F以及H矩阵的时候都会被使用到
    ...
    ​		由于名义状态的递推方程本质上是最优估计，不需要考虑噪声，对应方程为:
