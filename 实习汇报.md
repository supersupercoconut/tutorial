...
- **以FAST-LIO1为例，主要是分析IMU递推方程/LiDAR测量方程以及/去除运动畸变三者**。
  ...
  - IMU测量模型：IMU测量得到的角速度/加速度都是针对当前时间的Body系(或者是叫IMU系)的测量结果，不过由于加速度计会受到重力加速度的影响，在FAST-LIO中会通过静止30帧获取到一个重力加速度的估计值(即获取到全局系G中的重力值$g^G$), 计算$a_m$就非常方便。
    ...
    - IMU基本参数的确定: **需要确定的参数为实际重力加速度g，加计的偏置以及噪声，陀螺仪偏置以及噪声**。具体的初始化方案为静止初始化。
  ...
  - IMU递推方程: IMU作为LiDAR帧之间的状态估计工具，会涉及到状态以及协方差的递推。状态方面会选择误差状态向量以及名义状态量同时参与推导。**名义状态就是在递推过程中不包含噪声的估计结果, 误差状态向量即作为真实状态与预估状态(或称为名义状态)之间的差值**。名义状态会在每次计算F以及H矩阵的时候都会被使用到
    ...
    - 在实际推导过程中，涉及到非线性方程到线性方程的转换，即需要进行一阶线性化处理。在这里应该是有**三种推导方式**，主要在于线性化方式不同：
      ...
      3. 直接从连续状态推导误差状态的导数，再离散化获取结果**(推导上最简单，第一种方法实际上没有考虑到ESKF本身就是一种线性化点在零点附近，所以在数值上更加稳定的优势，直接在状态处离散化就与EKF相同，2、3这两种推导方法其线性化点都是对$\delta x$， 也就是0)**。首先计算各状态的导数形式。速度导数就是加速度，关于旋转的导数即使用右扰动模型进行推导，可以参考https://blog.csdn.net/weixin_44382195/article/details/110677858。
         $$
         \begin{aligned}
         	\dot{p}&=v\\
         ...
         $$
         \begin{aligned}
         	w&=\left[ n_{v}^{T}\quad n_{\theta}^{T}\quad n_{g}^{T}\quad n_{a}^{T} \right] ^T\sim N(0,Q)\\
...
- 状态更新:  核心是对于Kalman滤波中增益的计算，以及对真实状态的更新以及相关协方差的计算。$P_{pred}$作为IMU一直递推过程计算出来的协方差矩阵，H是h()对$\delta x$的雅可比矩阵，其中所使用R、t等属于状态向量的参数都是利用IMU方程递推到当前时刻的状态计算结果。

$$
\begin{aligned}
	K&=P_{pred}H^T\left( HP_{pred}H^T+V \right) ^{-1}\\
	\delta x&=K\left( z-h\left( x_t \right) \right)\\
