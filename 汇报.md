...
- 实际工厂环境下录制数据并且比较算法(比较算法中的问题比较大!!!)
- 全局地图增加噪声信息比较算法的鲁棒性(先给全局地图增加噪声信息进行处理) —— 已经完成了数据
- 点云退化检测部署 - 部署完毕，但是有一些参数仍需要后续调节，要不然切换的频率太频繁了
- gf++与gf建图效果对比
- 退化检测部署到gf++对比精度
- 整理数据集并且对比数据集的精度
...
- 关于退化检测部分的原理以及参数调节
  - 检测原理：利用其检测方法计算出来的特征向量(为一个1*6的向量)，但实际用于检测的部分是一个1维标量，按照论文中的描述，对该特征向量选择归一化，选择除去最大以及最小值之后向量中的最小值，如果该值越小就认为其发生了退化。
  - 调节参数：
    - voxel_size：从2m往下调节参数，发现退化检测参数的min值会直接下降了
    - 滤波参数：这里是为了避免轨迹切换的过于频繁
