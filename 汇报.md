...
- hm-lio测试匹配结果：
  ...
  - 将初始位姿配准方法与全局地图配准方法合并，并进行消融实验证明上述方法的可行性
...
- 3D Registration with Maximal Cliques这种匹配方式毕竟只是一种粗匹配方法，在实际仓库中进行测试发现在有些情况下匹配效果较差，故这里引入了一个ICP方法再进行精细化匹配，并且定性地分析了这种匹配结果的有效性。
