...
- 整理week6的工作汇报，在汇报的最开始的部分要写清楚(有一个内存占用的问题以及测量指标的修正问题)
- 了解控制(需要读完那篇nav-RL跟ego-planner)
- 全局地图增加噪声信息比较算法的鲁棒性(先给全局地图增加噪声信息进行处理) —— 已经完成了数据
...
- 对于MAC方法，github上目前提出了加速方法，可以将整体的速度增加80倍，并且在上面利用的GPU来进行计算，完美的适配nx。并且也可以去找如何使用一个Cpp框架来对python / cuda这些部分来进行计算。



- 关于MAC方法如何调节参数 | MAC是如何

  - MAC：是一种直接基于几何特征实现的点云配准方法
  - FAST-MAC：
  - MAC++：
...
- TODO: 
  - C++代码中如何调用python接口来控制学习类的数据进行加速处理 —— FASTMAC相当于对MAC方法进行了加速处理
  - 
...
- 初始加载19s 重写了初始化逻辑之后的结果
- 修正模块66s



- 目前需要在刚启动几帧做增强 所以LiDAR点从pushData()的降采样以及在optimize()中需要对初始逻辑进行修正
- nx上仍不能接受点云过多的情况，可以做进一步的降采样(目前限制2000)可以尝试限制到1500 —— 2000的平均耗时在75ms
- 点云的过滤最好还是按照体素来进行滤除而不是直接按照数量来进行滤除 | 但是直接降采样的对象就是从体素中获取到的结果





(nx的参数仍可以计算调试，并且可以尝试在降采样的部分将无人机启动位置处的点云进行更多的保留，后续流程可以依赖地图更新来保证后续的定位)



一些有误差的场景，即使使用MAC进行精细化的匹配也没有办法进行进一步的修正，但是不知道直接使用ICP这种方法能否解决：在初始匹配有误差的前提下，精匹配也无法完成修正

![image-20250626154234706](./figure/image-20250626154234706.png)
