...
- 满二叉树 | 完全二叉树 ： 满就是所有左右子树都有（除了底层，并且底层也是满的），完全二叉树除了底层都是满的，底层从左到右是来连续的。
...
- 高度 | 深度 (不同的教材对于初始的高度/深度定义成0或者1有些出入)
    - 高度是从叶子节点往根节点进行计数
    - 深度为根节点到叶子节点的计数
...
**递归方法相比于迭代方法的好处在于：通过函数不断返回值可以获取到二叉树中上一层的数据，但是迭代方法需要自行保存这个数据(比如使用stack)**
