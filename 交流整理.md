...
说明发布的话题只会进行可视化，内部数据的交换都是经过线程来进行处理的，并且没有使用图像信息
...
- 在原版ImMesh中提供了RGB色彩信息的渲染，但是感觉渲染是单独依靠lidar自己完成的
...
1. RGB_Voxel 管理空间中一个体素中的数据

    - voxel中的所有点云 ( 点位置直接对应grid的id )
    - 二维平面(应该是用于mesh生成的部分)
    - 以及三个向量来代表三个轴信息(short/mid/long)

2. Global_map  管理所有的点云+RGB_Voxel+KD_tree
...
3. Triangel_manager 管理与三角平面相关的部分
