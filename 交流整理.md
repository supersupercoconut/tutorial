...
出现了一个bug - 应该是在设置多线程的时候出现的问题 - shared_ptr出现了错误 显示一个共享指针 {use count -1610610478, weak count 32762}

![image-20240714210834245](figure/image-20240714210834245.png)

- 关闭cmakelists.txt的优化来避免出现变量 optimized_out的情况。但是cmakelist.txt中不使用优化 尤其是从O3降到O0之后，整个系统的实时性基本上是没有的。

- 这里还是由于voxel_ptr指向的内容有问题，导致这个智能指针的use count以及weak count出现错误 —— 针对use count的异常处理，设置了一个0~1000的合理范围，超过这个范围即认为fault，直接return.

![image-20240715001805245](figure/image-20240715001805245.png)



- 还有一点关于cmakelists.txt中设置debug或者release模块的情况下会导致的错误

release | 并且目前导致多线程出现问题的部分基本上都存在在渲染部分 —— 原作者自己的部分基本没有
