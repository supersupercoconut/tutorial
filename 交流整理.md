...
- r3live中的点云颜色获取 —— 都是使用点云在当前帧中的投影位置(周围RGB像素差值得到的)。除去第一帧之外的所有点的RGB更新都是利用贝叶斯公式进行的更新。

    

现在rosbag中获取camera与lidar信息对应好的数据 | **之前说的人的脚面这些剔除效果不好的情况，对于velodyne这种比较稀疏的点云雷达，实际上是扫描不出来这些点的**
...
整理一下r3live | 并且r3live中直接使用的原始点云没有使用fastlio中的特征点云
...
1. 多帧点云获取以及与image信息联合使用

    - 发现immesh里面也需要里程计信息才能做camera+lidar的对齐，而转换之后的部分也是没有办法这个里程计数据 ( 所以这里还是使用r3live的原始框架上做改动——改成获取里程计信息跟点云信息然后上色就行 )

    - 需要从immesh的数据中获取数据并进行处理 —— LidarMeasureGroup中包含了一帧点云数据 并且包含了一个
2. 重新读r3live已经看到到VIO_update了
4. Kalman暑假之前要弄完
